...
M._ls_dir_unix = function(directory)
  local command = string.format("find %s -maxdepth 1 -type f", directory)
  local handle, err = io.popen(command, "r")
  if not handle or err then
    M.log:error("Could not execute command, got error: '%s'", err)
    return {}
  end
...
---
---Returns a list of absolute file paths in the directory. Uses system commands
---(`dir` or `find`).
---
---@param directory string Absolute path to directory.
---@return string[]|nil files List of file paths, or nil if inaccessible.
M.ls_dir = function(directory)
  return cache.compute_cached("fs.ls-dir", function()
    if M.platform().is_linux then
      return M._ls_dir_unix(directory)
    end

    local filename = M.basename(directory) .. ".txt"
    local cmd = 'cmd /C "dir %s /B /S > %s"'
    local tempfile = M.join_path(ogetenv "TEMP", filename)
    ...
    if file then
      ...
      if not success then
        return M.log:error "[ls_dir] Unable to create temp file!"
      end
